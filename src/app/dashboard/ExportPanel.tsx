...
export default function ExportPanel({ users }: { users: UserRow[] }) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      a.href = url;
      a.download = `nutriscan_informe_${new Date().toLocaleDateString('es-AR', { timeZone: 'America/Argentina/Buenos_Aires', day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '-')}.xlsx`;
      a.click();
  ...
  return (
    <div className="rounded-2xl bg-white shadow-sm overflow-hidden mt-6">
      <div className="border-b px-6 py-4 flex items-center justify-between gap-4">
        <div>
          <h3 className="font-semibold text-gray-900">Exportar Informe</h3>
