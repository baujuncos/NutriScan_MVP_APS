...
export default function RegisterForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const role = isInvestigador ? 'investigador' : determineRoleFromEmail(data.email);
    ...
    if (authData.user) {
      ...
      if (profileError) {
        setServerError('Error al crear el perfil. Intente nuevamente.');
        return;
      }
    }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          {!isInvestigador && (
            <p className="text-xs text-gray-500">
              Si tu email es <strong>@ucc.edu.ar</strong> serás registrado como deportista UCC.
              De lo contrario, como usuario particular.
            </p>
          )}
